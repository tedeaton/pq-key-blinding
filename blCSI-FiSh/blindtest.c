...
int main(){
	...
	unsigned char *blinded_pk = aligned_alloc(64,PK_BYTES);
	...
	unsigned char *blinding_nonce = aligned_alloc(64,SK_BYTES);
	RAND_bytes(blinding_nonce, SEED_BYTES);
	...
	for(int i=0 ; i<KEYS; i++){
		...
		t0 = clock();
		t = rdtsc();
		blind_public_key(blinded_pk, pk, blinding_nonce);
		blindCycles += rdtsc()-t;
		blindTime += 1000. * (clock() - t0) / CLOCKS_PER_SEC;


		for(int j=0; j<SIGNATURES_PER_KEY; j++){
			printf("blinded signature #%d for key %d \n", j , i );

			t0 = clock();
			t = rdtsc();
			blinded_sign(sk,message,1,sig,&sig_len,blinding_nonce);
			signCycles += rdtsc()-t;
			...
			t0 = clock();
			t = rdtsc();
			int ver = csifish_verify(blinded_pk,message,1,sig, sig_len);
